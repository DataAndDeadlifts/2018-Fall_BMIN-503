...
obesity <- read.csv(url("https://raw.githubusercontent.com/HimesGroup/BMIN503/master/DataFiles/county_obesity_prevalence.csv"))
...
counties_merged <- left_join(counties, select(obesity, state, GEO_ID, age.adjusted.percent.2004, age.adjusted.percent.2014))
```
...
prev_min <- min(as.numeric(counties_merged$age.adjusted.percent.2004),as.numeric(counties_merged$age.adjusted.percent.2014))
prev_max <- max(as.numeric(counties_merged$age.adjusted.percent.2004),as.numeric(counties_merged$age.adjusted.percent.2014))
...
> Judging by this code it looks like 2014 is worse overall than 2004 in almost every state except Colorado. The southeast US looks to have gotten much more obese than many other sections of the continental US over the same time span, but overall America is growing more obese overall.
