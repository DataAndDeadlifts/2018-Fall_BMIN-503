...
> ## Results
> The original setup of the problem and its answer resulted in a model that predicted exactly what I was asking it to predict. In short I asked the model to predict whether any protein as a whole produced amyloid and with 11+ million records of sometimes over 3000 entries for a single protein - it did rather well.

> What I'm more interested in is any given section of protein producing amyloid, which is what I am working on now (and hopefully will finish by my presentation). To achieve a model that does this I am adding many more ngram level features to balance the data between protein-level features and ngram-level features. I am also changing the prediction output to give me an array of n-predictions. In my case the output could be [1, 0, 0, 1, 1] which would mean the first and last 2 amino acids are predicted as amyloid.
...
> It looks like the samples vary quite a bit, they all seem to have really high error bars too. In terms out 'outliers' I might point at GSM692115 for being high, GSM692118 for being lowand GSM69219 for being high - its hard to really say for certain though.
...
> This looks a lot more uniform! Thanks RMA ;)
...
> The top gene is "NQO1" and it has 3 total probes associated in the most significant results. The above probe is the most differentially expressed one in the dataset.
